...
export class EventManager {
  ...
  onIntegrationEvent(evt) {
    ...
    if (evtServerId && evtServerId !== this.getCurrentServerId())
        return;
  ...
  callComponentServiceWithServerId(name, args, notifyOnError = false) {
  ...
  // Returns: 
  //  - void (this is a fire-and-forget HAOS integration call)
  callComponentService(name, args, notifyOnError = false) {
  ...
  // Returns: 
  //  - void (this is a fire-and-forget HAOS integration call)
  callMixedDomainService(name, args, notifyOnError = false) {
    if (!this.getHass()) {
  ...
  // Returns: 
  //  - void (this is a fire-and-forget HAOS integration call)
  callService(domain, name, args, notifyOnError = false) {
    ...
    return hass.callService(domain, name, args, undefined, notifyOnError).catch((err) => {

      // Log raw error
      if (this.getLogger().isErrorEnabled()) console.error(...this.getLogger().error(`callService(domain, name, args, notifyOnError): raw error:`, domain, name, args, notifyOnError, err));
