...
class TrackpadCard extends HTMLElement {
  ...
  setConfig(config) {
    // No configuration available for this card
  }
  ...
  async connectedCallback() {
    console.log("Trackpad - connectedCallback");
  ...
  set hass(hass) {
    console.log("Trackpad - set hass():", hass);
    this._hass = hass;
    if (!this._uiBuilt) {
      this.buildUi(this._hass);
    }
  ...
  buildUi(hass) {
    if (this._uiBuilt) {
      console.log("Trackpad - buildUi() SKIPPED");
      return;
    }
    console.log("Trackpad - buildUi() ENTER");
